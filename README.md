...
현재 공모전 MVP의 코스 추천과 러닝 코치는 학습형 모델이 아니라, 판단 근거를 확인할 수 있는 **규칙·점수 기반 방식**으로 동작합니다. 실시간 교통·공사·재해 데이터까지 통합하는 것이 최종 방향이며, 현재 버전에서는 전국 시군구 단위로 조회하는 공공데이터포털의 사고 이력을 활용한 안전 경로 추천을 우선 구현했습니다.

## 앱 실행 화면

| 안전 코스 추천 | 실시간 러닝 안내 | 기록 분석 |
|---|---|---|
| ![안전 코스 추천](docs/images/course-recommendation.jpeg) | ![실시간 러닝 안내](docs/images/running-session.png) | ![기록 분석](docs/images/record-detail.jpeg) |
...
5. **설명 가능하고 확장 가능한 오픈소스 구현**  
   추천 결과의 기준을 코드와 문서로 확인할 수 있게 만들고, 향후 안전 데이터 종류·실시간 도로 정보·개인화 모델을 추가할 수 있는 구조를 지향합니다.
...
현재 MVP는 공공데이터포털의 **2024년 보행노인 교통사고 다발지역 데이터**를 활용합니다. 출발 좌표를 Kakao Local API의 법정동 코드로 변환하고, 앞 2자리의 시도 코드와 다음 3자리의 시군구 코드를 추출해 해당 지역의 사고다발지역을 동적으로 조회합니다. 따라서 특정 도시명을 코드에 고정하지 않고 대한민국 시군구 단위의 요청 구조를 지원합니다.

단순히 위험 지점을 지도 위에 표시하는 데서 끝내지 않고, 각 위험 지점과 추천 경로 사이의 최단거리를 계산해 해당 지점이 실제 러닝 코스에 영향을 주는지 판단합니다. 지역 또는 연도에 따라 API가 데이터를 제공하지 않거나 요청에 실패하면 안전 점수를 임의로 생성하지 않고 `정보 없음`으로 표시합니다.
...
현재 위치로 코스를 만들 때는 러닝 시작 직전 Expo Location에서 최신 좌표를 다시 받아 정확도를 검증합니다. 검증된 원본 좌표를 코스 출발 입력과 러닝 세션의 첫 위치에 함께 사용해 지도에 보이는 출발점과 실제 기록 시작점의 차이를 줄였습니다.

검색한 장소를 출발지로 선택한 경우에는 GPS 갱신이 지도 카메라를 현재 위치로 되돌리지 않도록 분리했습니다. 코스가 생성되면 출발점·반환점·전체 경로가 한 화면에 들어오도록 영역을 맞추고, 사용자가 현재 위치 버튼을 눌렀을 때만 GPS 위치로 이동합니다.
...
| 공공데이터포털 | 사고 다발지역 좌표와 사고 통계 제공 | 출발지 시군구 데이터를 동적으로 조회·정규화한 뒤 경로와의 거리 계산 |
...
- 출발지 법정동 코드에서 시도·시군구 코드를 추출해 전국 시군구 단위 사고 데이터 조회
- 사고 이력 기반 안전 점수·등급·경고 마커와 데이터 미제공 상태 표시
...
- **데이터 범위:** 전국 시군구 단위 조회 구조를 지원하지만, 2024년 보행노인 교통사고 다발지역 한 종류만 사용하며 지역에 따라 제공되는 데이터가 없을 수 있습니다.
...
- **지역 판별:** 법정동 코드가 유효하지 않거나 행정구역·공공데이터 조회에 실패하면 안전도를 임의로 계산하지 않고 `정보 없음`으로 표시합니다.
...
- 서로 다른 시군구에서 행정구역 코드 변환과 지역별 안전 데이터 요청 확인
- 검색 출발지에서 GPS 갱신으로 지도 카메라가 이동하지 않고 전체 코스를 유지하는지 확인
...
1. 보행어린이·보행자 사고와 범죄·보행 환경 등 전국 안전 데이터 종류 확대
